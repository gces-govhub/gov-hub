...
def organize_existing_siafi_data():
    ...
    # Organizar arquivo principal do SIAFI
    source_file = Path("data/raw/202501_Despesas_2025-06-30.csv")
    if source_file.exists():
        dest_file = raw_dir / "siafi_despesas_execucao_202501_2025-06-30.csv"
        if not dest_file.exists():
            shutil.copy2(source_file, dest_file)
            logger.info(f"📄 Copiado: {dest_file.name}")
        
        # Criar amostra para análise
        create_sample(dest_file, processed_dir)
        
        # Gerar relatório básico
        generate_basic_report(dest_file, reports_dir)
